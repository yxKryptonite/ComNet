...
我们计算了传输过程中的吞吐率和丢包率，用 vmaf 计算了传输后视频的 vmaf 指标，并将结果可视化。

接着，我们对实验结果进行了分析，发现视频传输中存在帧不对齐的情况，在进行手动对齐操作之后，我们发现 vmaf 指标取得了显著的提高。
...
此外，AlphaRTC 的某个以来 depot_tools 在某个版本更新之后就去掉了 ninja 包管理器的安装，但 AlphaRTC 需要 ninja 来安装其他的依赖，所以我们更改了 `Dockerfile.compile` ，手动安装 ninja。
...
此外，我们还发现，相对于同一IP下的传输，不同IP下的传输，三个 psnr 的值都有了很大幅度的下降，这说明不同IP下的传输，单帧的质量损失更加严重，这从输出视频画面更加模糊可以直观的看出。
